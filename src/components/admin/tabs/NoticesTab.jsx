...
export default function NoticesTab() {
  ...
  const author = user?.nickName || user?.name || ''; // 작성 코드와 동일한 기준
  ...
  // NoticeMain의 안전한 URL 생성 방식과 맞춤: 목록은 /api/notices/paged, 쓰기는 /api/notices
  const apiBase = () => {
    const base = typeof import.meta !== 'undefined' ? (import.meta.env?.VITE_API_URL || '') : '';
    const origin = base.trim() ? base.trim().replace(/\/+$/, '') : '';
    return origin || '';
  };
  const listUrl = () => `${apiBase()}/api/notices/paged`;   // 목록 조회
  const crudUrl = () => `${apiBase()}/api/notices`;         // 생성/수정/삭제
  ...
  const fetchNotices = async () => {
    ...
    try {
      ...
      // NoticeMain 응답 형태 대응(content 또는 data) 
      const body = res.data;
  ...
  const remove = async (id) => {
    ...
    try {
      ...
      // 백엔드 204(No Content) 대응: res.data가 비어 있어도 성공
